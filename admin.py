    ...
    try:
        ...
        return jsonify({
            ...
            'users': [{
                ...
                'name': r[2], 
                'phone': r[3], 
                'regDate': str(r[4]) if r[4] else '', 
                'approved': r[5], 
                'role': r[6] or 'normal',
                'productScoreUsed': r[7] or 0,
                'brandSalesUsed': r[8] or 0,
                'productCount': r[9]
            } for r in rows]
        })
...
def reset_usage():
    ...
    d = request.json
    user_id = d.get('userId')
    usage_type = d.get('type')  # 'product_score', 'brand_sales', 또는 None(전체)
    
    try:
        reset_user_usage(user_id, usage_type)
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)})
